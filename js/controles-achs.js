...
(function ($) {
  ...
  $(document).ready(function () {
	    ...
	    //Pestanhas
	  	$(".component-navigator-menu").tabACHS();
...
//  			});
	  ...
	  /*$('.boxCalendar').datepicker(calendar_settings).click(function() {
		...
		*/
		...
		$(".boxCalendar").click(function () {
		  calendarioAbierto = $(this).parent().children('.boxCalendar').datepicker(calendar_settings);
		  calendarioAbierto.datepicker('show');
		  posicionarCalendario($(this).parent());
	  });
    ...
    //Draggable 
    if ($(".draggableComponent").tableDnD) 
    	$(".draggableComponent").tableDnD({
   ...
   //Draggable Table (out)
   $('.draggableDataComponent div').each(function(i, elemento){
                $(elemento).draggable({
                    revert: 'invalid', 
                    helper: 'clone',
                    cursor: 'pointer',
	...
	//Dropable Content
		function initDropable(){
            ...
            $('#a-include-all-first').unbind('click').click(function(){
                $('#first-content div').each(function(i, firstHtml){
                    ...
                    if ($('.dropableComponent').find('#' + firstElement.attr('id')).length == 0) 
                        putElementsIntoDropContainer(firstElement, $('.dropableComponent'))
                });
            });
            ...
            $('#a-include-all-second').unbind('click').click(function(){
                $('#second-content div').each(function(i, secondtHtml){
                    ...
                    if ($('.dropableComponent').find('#' + secondElement.attr('id')).length == 0) 
                        putElementsIntoDropContainer(secondElement, $('.dropableComponent'))
                });
            });
        }
        ...
        function quitarElemento(id){
            var elementoAEliminar = $('.dropableComponent').find('#' + id);
            ...
            var checkEmptyGroupsContainer = function(){
                if (parseInt($('#span-count-first').html()) == 0 &&
                    ...
                    $('.dropableComponent').parent().addClass('empty');
                    $('.dropableComponent h4').hide();
